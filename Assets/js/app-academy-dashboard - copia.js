...
reporteVendidos();
    ...
    http.onreadystatechange=function(){
      if (this.readyState==4 && this.status==200) {
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          cantidad.push(res[i][23, 35, 10, 20, 35, 23]);
        }
  ...
  function reporteVendidos(){
    ...
    const url = base_url+"Dashboard/reporteVendidos";
    const http =new XMLHttpRequest();
    http.open("POST", url, true);
    http.send();
    http.onreadystatechange=function(){
  ...
  if (dt_academy_course.length) {
    ...
    $('div.head-label').html('<h5 class="card-title mb-0 text-nowrap">Lista de Nuestros Usuarios</h5>');
  }
